...
def force_coeffs_sliding_average(history_force_Coeffs: pd.DataFrame, sliding_time_interval: float, delta_t: float):
    sampling_num = int(sliding_time_interval / delta_t)
    if history_force_Coeffs.shape[0] <= sampling_num:
        sliding_average_cd = np.mean(signal.savgol_filter(history_force_Coeffs.iloc[:, 2],49,0))
        sliding_average_cl = np.mean(signal.savgol_filter(history_force_Coeffs.iloc[:, 3],49,0))
    else:
        sliding_average_cd = np.mean(signal.savgol_filter(history_force_Coeffs.iloc[-sampling_num:, 2],49,0))
        sliding_average_cl = np.mean(signal.savgol_filter(history_force_Coeffs.iloc[-sampling_num:, 3],49,0))
    return sliding_average_cd, sliding_average_cl


def sliding_history_force_coeffs(history_force_Coeffs: pd.DataFrame, sliding_time_interval: float, delta_t: float):
    sampling_num = int(sliding_time_interval / delta_t)
    if history_force_Coeffs.shape[0] <= sampling_num:
