...
class OpenFoam_tianshou(gym.Env):
    ...
    def reward_function(self):
        ...
        print(
            1.549 - drug_coeffs_sliding_average,
    ...
    def force_coeffs_sliding_average(self, sliding_time_interval):
        ...
        if self.history_force_Coeffs_df.shape[0] <= sampling_num:
            sliding_average_cd = np.mean(
                signal.savgol_filter(self.history_force_Coeffs_df.iloc[:, 2], 25, 0)
            )
            sliding_average_cl = np.mean(
                signal.savgol_filter(self.history_force_Coeffs_df.iloc[:, 3], 25, 0)
            )
        else:
            sliding_average_cd = np.mean(
                signal.savgol_filter(
                    self.history_force_Coeffs_df.iloc[-sampling_num:, 2], 25, 0
                )
            )
            sliding_average_cl = np.mean(
                signal.savgol_filter(
                    self.history_force_Coeffs_df.iloc[-sampling_num:, 3], 25, 0
                )
            )
