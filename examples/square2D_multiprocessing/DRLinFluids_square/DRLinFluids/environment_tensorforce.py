...
class OpenFoam_tensorforce(Environment, metaclass=ABCMeta):
    ...
    def states(self):
        ...
        if self.state_params["type"] == "pressure":
            return dict(
                type="float", shape=(int(self.state_params["probe_info"].shape[0]),)
            )
    ...
    def force_coeffs_sliding_average(self, sliding_time_interval):
        ...
        if self.history_force_Coeffs_df.shape[0] <= sampling_num:
            sliding_average_cd = np.mean(
                signal.savgol_filter(self.history_force_Coeffs_df.iloc[:, 2], 49, 0)
            )
            sliding_average_cl = np.mean(
                signal.savgol_filter(self.history_force_Coeffs_df.iloc[:, 3], 49, 0)
            )
        else:
            sliding_average_cd = np.mean(
                signal.savgol_filter(
                    self.history_force_Coeffs_df.iloc[-sampling_num:, 2], 49, 0
                )
            )
            sliding_average_cl = np.mean(
                signal.savgol_filter(
                    self.history_force_Coeffs_df.iloc[-sampling_num:, 3], 49, 0
                )
            )
