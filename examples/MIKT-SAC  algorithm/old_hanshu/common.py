    ...
    * hidden_sizes[-1] * output_dim
    ...
    def __init__(
        ...
        for in_dim, out_dim, norm, activ in zip(
            ...
            # print('1',in_dim, out_dim, norm, activ)
            model += miniblock(in_dim, out_dim, norm, activ, linear_layer)
        if output_dim > 0:  #未使用
            model += [linear_layer(hidden_sizes[-1], output_dim)]
        self.output_dim = output_dim or hidden_sizes[-1]
        self.model = nn.Sequential(*model)    # print(self.model) #[(3，512),(512，128)]   [(4，512),(512，128)]   无输出层
...
class Net(nn.Module):
    ...
    def forward(
        ...
        logits,obs_all = self.model(obs)   # 一定=MLP中的forward函数的输出  [256,128]
        self.cishu+=1
