...
class Critic(nn.Module):
    ...
    def __init__(
        ...
        # print(type(self.last),self.last)    #[128, 1]
    ...
    def forward(
        ...
        #         dtype=torch.float32,
        #     ).flatten(1)
        #     # print(np.array(obs).shape,np.array(act).shape)  #act
        #     obs = torch.cat([obs, act], dim=1)   #在给定维度中连接给定的 seq 张量序列
        # logits, hidden = self.preprocess(obs)    #处理的关键，初始定义的net网络， 与net中的forward函数的输出相同 网络层1 [(4，512),(512，128)]
        logits, hidden,vf_student_ps, embedding = self.preprocess(obs,act=act)  # 处理的关键，初始定义的net网络， 与net中的forward函数的输出相同 网络层1 [(4，512),(512，128)]
        variational_mu = self.preprocess.variational_net(embedding)   #24到120
        #sac learn()中运行四次  obs[256 4] -> [256,128]   4-> 128
        logits = self.last(logits)          #网络层2   [256, 1]   处理两次logits  # sac learn()中运行四次  [256,128]  *[128 1] 变为 [256,1]  128-> 1
        # return logits
        return logits, vf_student_ps, embedding,variational_mu ,self.preprocess.variational_logstd
...
class ActorProb(nn.Module):
    ...
    :param preprocess_net: a self-defined preprocess_net which output a
        flattened hidden state.  输出扁平隐藏状态的自定义preprocess_net
    :param action_shape: a sequence of int for the shape of action.
    :param hidden_sizes: a sequence of int for constructing the MLP after
        preprocess_net. Default to empty sequence (where the MLP now contains
        only a single linear layer).
    ...
    def __init__(
        ...
        self.device = device
        self.output_dim = int(np.prod(action_shape))
        input_dim = getattr(preprocess_net, "output_dim", preprocess_net_output_dim)   #256

        self.mu = MLP(    #在这里加入输出层
            input_dim,    # type: ignore
            self.output_dim,
            hidden_sizes,
            device=self.device
    ...
    def forward(
        ...
        # logits, hidden = self.preprocess(obs, state)   #网络层1 [(3，512),(512，128)]
        logits, hidden, student_ps, embedding = self.preprocess(obs, state=state)  # 网络层1 [(3，512),(512，128)]
        variational_mu = self.preprocess.variational_net(embedding)
        # print('2', obs.shape, logits.shape)   # [256,128]   3->128
        mu = self.mu(logits)                   #网络层2   [256, 1]   依据logits输出mu
        # print('3',  mu.shape)    #  [256,128] *[128 1] 变为 [256,1]    128-> 1
        if not self._unbounded:
            mu = self._max * torch.tanh(mu)
        if self._c_sigma:  #未进入
        ...
        self.sigma=sigma
        self.hidden=state
        # return (mu, sigma), state   #ppo
        # return mu
        return mu, student_ps, embedding,variational_mu ,self.preprocess.variational_logstd     #sac
...
class RecurrentActorProb(nn.Module):
    ...
    def forward(
        ...
        logits = obs[:, -1]
        #logits torch.Size([10, 128])
        mu = self.mu(logits)
        #mu torch.Size([10, 1]
        # print('1', mu, '1', mu.shape)
        if not self._unbounded:
            mu = self._max * torch.tanh(mu)
        if self._c_sigma:
            sigma = torch.clamp(self.sigma(logits), min=SIGMA_MIN, max=SIGMA_MAX).exp()
            ...
            shape[1] = -1
            sigma = (self.sigma_param.view(shape) + torch.zeros_like(mu)).exp()
            # print('1', sigma, '1', sigma.shape)
        # please ensure the first dim is batch size: [bsz, len, ...]
        # print(mu,type(mu),sigma,type(sigma))
        return (mu, sigma), {
...
class RecurrentCritic(nn.Module):
    ...
    def __init__(
        ...
        # print(hidden_layer_size + int(np.prod(action_shape)))
    ...
    def forward(
        ...
        # print(len( obs.shape), obs.shape)
        # 需要堆栈层数,VectorReplayBuffer(args.buffer_size, len(train_envs), stack_num=3)
        # obs [bsz, len, dim] (training) or [bsz, dim] (evaluation)
        # In short, the tensor's shape in training phase is longer than which
        # in evaluation phase.
