...
# from hanshu.buffer.base import ReplayBuffer #可能需要改变
# from hanshu.buffer.vecbuf import VectorReplayBuffer
...
class Collector(object):
    ...
    def reset_env(self, gym_reset_kwargs: Optional[Dict[str, Any]] = None) -> None:
        ...
        rval = self.env.reset(**gym_reset_kwargs)    #该句调用reset
        # print('reset',self.env,**gym_reset_kwargs)
        # print(rval)
    ...
    def collect(
        ...
        while True:
            ...
            # step in env         reset能调用   但step不行
            # result = self.env.step(action_remap, ready_env_ids)  # type: ignore  #这里最关键 可以绕开tianshou的worker   需要测试
        ...
        # print('collect',self.collect_step)
