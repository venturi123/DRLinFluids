...
# from hanshu.worker import DummyEnvWorker
from hanshu.worker import SubprocEnvWorker
# from hanshu.worker import DummyEnvWorker,RayEnvWorker,SubprocEnvWorker,EnvWorker   #此处有问题   导致84行类型不一致
...
class BaseVectorEnv(object):
    ...
    def __init__(
        ...
        print(self.worker_class, type(EnvWorker),issubclass(self.worker_class, EnvWorker))
    ...
    def step(
        ...
        if not self.is_async:
            ...
            for i, j in enumerate(id):
                # print(self.workers[j])
                self.workers[j].send(action[i])    #应该是没法调用方法的原因 无法传递到step   或者说转移到这台机未能设置的envs   试着绕过这一步
            result = []
            for j in id:
