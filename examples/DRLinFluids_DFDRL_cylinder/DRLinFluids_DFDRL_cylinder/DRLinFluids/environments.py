...
class OpenFoam(gym.Env):
    def __init__(
        ...
        **kwargs,
        ...
        if server:
            ...
            pd.DataFrame(action_tocsv_list).to_csv(
                self.foam_root_path + "/system/jet.csv", index=False, header=False
            )
            ...
            self.velocity_table_init = utils.read_foam_file(
                foam_root_path + "/postProcessing/probes/0.000/U",
                dimension=self.foam_params["num_dimension"],
            )
            cfd_init_time = int(
                self.foam_params["cfd_init_time"]
                - self.agent_params["interaction_period"]
            )
            self.pressure_table_init = utils.read_foam_file(
                foam_root_path + "/postProcessing/probes/0.000/p",
                dimension=self.foam_params["num_dimension"],
            )
            self.initial_force_Coeffs_df = utils.read_foam_file(
                foam_root_path + "/forceCoeffsIncompressible/0.000/forceCoeffs.dat"
            )
            self.pressure_DMD_initial = utils.read_foam_file(
                foam_root_path + "/probes/0.000/p",
                dimension=self.foam_params["num_dimension"],
            )
            self.pressure_DMD_initial = (
                self.pressure_DMD_initial.iloc[1:, 1:].to_numpy().T
            )
            self.pressure_DMD_initial_snapshot = self.pressure_DMD_initial[
                :,
                (
                    int(
                        self.agent_params["interaction_period"]
                        / self.foam_params["delta_t"]
                    )
                    - 1
                ) : (
                    int(
        ...
        self.dmd_state = np.zeros(
            (int(self.state_params["probe_info"].shape[0] + 1), 30), dtype=int
        )
        if self.state_params["type"] == "pressure":
            self.observation_space = spaces.Box(
                low=-np.Inf, high=np.Inf, shape=(self.dmd_state.shape), dtype=np.float32
            )
    ...
    def step(self, actions: np.ndarray):
        ...
        if self.num_trajectory < 1.5:
            self.start_time_float = np.around(
                float(self.cfd_init_time_str), decimals=self.decimal
            )
        else:
            self.start_time_float = self.end_time_float
        self.end_time_float = np.around(
            self.start_time_float + self.action_time, decimals=self.decimal
        )

        # action
        self.control_matrix_gammaDMDc = self.control_matrix_gammaDMDc.flatten()
        self.control_matrix_gammaDMDc = np.delete(
            self.control_matrix_gammaDMDc, 0, axis=0
        )
        self.control_matrix_gammaDMDc = np.append(
            self.control_matrix_gammaDMDc, actions
        )
        ...
        if self.num_trajectory < 1.5:
            ...
            action_tocsv_list = [
                [self.start_time_float, 0, 0, 0],
                [self.end_time_float, 0, self.actions_sequence[-1], 0],
            ]
            pd.DataFrame(action_tocsv_list).to_csv(
                self.foam_root_path + "/system/jet.csv", index=False, header=False
            )
            ...
            action_tocsv_list = [
                [self.start_time_float, 0, self.actions_sequence[-2], 0],
                [self.end_time_float, 0, self.actions_sequence[-1], 0],
            ]
            pd.DataFrame(action_tocsv_list).to_csv(
                self.foam_root_path + "/system/jet.csv", index=False, header=False
        ...
        cfd.run(
            ...
            self.foam_params,
            self.action_time,
            self.agent_params["purgeWrite_numbers"],
            self.action_time,
        ...
        if self.num_trajectory > 1.5:
            self.start_time_float = format(
                self.end_time_float - self.action_time, ".3f"
            )  # 更改
        ...
        self.force_Coeffs_df = utils.read_foam_file(
            self.foam_root_path
            + f"/postProcessing/forceCoeffsIncompressible/{self.start_time_float}/forceCoeffs.dat"
        )
        ...
        # DF-DRL
        if self.num_trajectory < 1.5:
            self.pressure_DMD_initial_snapshot = np.delete(
                self.pressure_DMD_initial_snapshot_init, 0, axis=1
            )
            self.pressure_DMD_initial_snapshot = np.hstack(
                (
                    self.pressure_DMD_initial_snapshot,
                    next_state.reshape(
            ...
            self.pressure_DMD_initial_snapshot = np.delete(
                self.pressure_DMD_initial_snapshot_step, 0, axis=1
            )
            self.pressure_DMD_initial_snapshot = np.hstack(
                (
                    self.pressure_DMD_initial_snapshot,
        ...
        self.pressure_DMD_initial_snapshot = scaler.fit_transform(
            self.pressure_DMD_initial_snapshot.T
        )
        ...
        if self.num_trajectory > 1.5:
            self.start_time_float = np.around(
                self.end_time_float - self.action_time, decimals=self.decimal
            )
        ...
        return (
            ...
            terminal,
            {
                "vortex_shedding_cd_mean": cd_mean,
                "vortex_shedding_cl_mean": cl_mean,
                "vortex_shedding_cd_std": cd_std,
                "vortex_shedding_cl_std": cl_std,
                "action": self.actions_sequence[-1],
            },
        )
    ...
    def reward_function(self):
        ...
        drug_coeffs_sliding_std = self.force_coeffs_sliding_std(vortex_shedding_period)[
            0
        ]
        lift_coeffs_sliding_std = self.force_coeffs_sliding_std(vortex_shedding_period)[
            1
        ]
        print(
            self.agent_params["cd_0"] - drug_coeffs_sliding_average,
            lift_coeffs_sliding_std,
        )

        return (
            drug_coeffs_sliding_average,
            np.abs(lift_coeffs_sliding_average),
            drug_coeffs_sliding_std,
            lift_coeffs_sliding_std,
        )
    ...
    def reset(self):
        if self.num_episode < 0.5:
            ...
            pd.DataFrame(self.episode_reward_sequence).to_csv(
                self.foam_root_path + "/record/total_reward.csv",
                index=False,
                header=False,
            )
            if self.episode_reward_sequence[-1] == np.max(self.episode_reward_sequence):
                pd.DataFrame(self.actions_sequence).to_csv(
                    self.foam_root_path + "/record/best_actions.csv",
                    index=False,
                    header=False,
                )
                pd.DataFrame(self.history_force_Coeffs_df).to_csv(
                    self.foam_root_path + "/record/best_history_force_Coeffs_df.csv",
                    index=False,
                    header=False,
                )
        ...
        self.all_episode_actions.to_csv(
            self.foam_root_path + "/record/all_episode_actions.csv",
            index=False,
            header=False,
        )
        self.all_episode_trajectory_reward.to_csv(
            self.foam_root_path + "/record/all_episode_trajectory_reward.csv",
            index=False,
            header=False,
        )
        self.history_force_Coeffs_df.to_csv(
            self.foam_root_path
            + f"/record/history_force_Coeffs_df_{self.num_episode}.csv",
            index=False,
            header=False,
        )
        ...
        scaler = StandardScaler()
        self.pressure_DMD_initial_snapshot = scaler.fit_transform(
            self.pressure_DMD_initial_snapshot_init.T
        )
        self.control_matrix_gammaDMDc = self.control_matrix_gammaDMDc.reshape((30, 1))
        self.pressure_DMD_initial_snapshot = np.hstack(
            (self.pressure_DMD_initial_snapshot, self.control_matrix_gammaDMDc)
    ...
    def force_coeffs_sliding_average(self, sliding_time_interval):
        ...
        self.history_force_Coeffs_df.iloc[
            -self.history_force_Coeffs_df_stepnumber :, 2
        ] = signal.savgol_filter(
            self.history_force_Coeffs_df.iloc[
                -self.history_force_Coeffs_df_stepnumber :, 2
            ],
            int(self.history_force_Coeffs_df_stepnumber / 2),
            0,
        )
        self.history_force_Coeffs_df.iloc[
            -self.history_force_Coeffs_df_stepnumber :, 3
        ] = signal.savgol_filter(
            self.history_force_Coeffs_df.iloc[
                -self.history_force_Coeffs_df_stepnumber :, 3
            ],
    ...
    def force_coeffs_sliding_std(self, sliding_time_interval):
        sampling_num = int(
            self.history_force_Coeffs_df_stepnumber
            / self.agent_params["action_discount"]
        )
        if self.history_force_Coeffs_df.shape[0] <= sampling_num:
            sliding_average_cd_std = np.std(self.history_force_Coeffs_df.iloc[:, 2])
            sliding_average_cl_std = np.std(self.history_force_Coeffs_df.iloc[:, 3])
        else:
