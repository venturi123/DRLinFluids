...
def validation(
    ...
    forceCoeffs_path = (
        root_path + "/postProcessing/forceCoeffsIncompressible/0/forceCoeffs.dat"
    )
    ...
    if "interval" in const:
        ...
        if np.max(const["interval"]) <= 1:
            interval = list(
                map(int, forceCoeffs_data.shape[0] * np.array(const["interval"]))
            )
        else:
            interval = const["interval"]
    else:
    ...
    Cl_RMS = np.sqrt(
        np.sum(forceCoeffs_data[interval[0] : interval[1], 3] ** 2)
        / num_selected_points
    )
    ...
    if coeffs_figure:
        ...
        Cp = (probe_data[interval[0] : interval[1], 1:] - p0) / (
            0.5 * rho * const["U0"] * const["U0"]
        )
    ...
    if time_figure:
        ...
        axes0[0].plot(
            Time,
            [Cd_mean for _ in range(len(Time))],
            "r--",
            label=f"mean = {np.around(Cd_mean, decimals=3)}",
        )
        ...
        axes0[1].plot(
            Time,
            [Cl_mean for _ in range(len(Time))],
            "r--",
            label=f"mean = {np.around(Cl_mean, decimals=3)}",
        )
        axes0[1].plot(
            Time,
            [Cl_RMS for _ in range(len(Time))],
            "g--",
            label=f"RMS = {np.around(Cl_RMS, decimals=3)}",
        )
    ...
    if freq_figure:
        ...
        axes1[0].set_title(
            f"shedding_frequency = {np.around(shedding_frequency, decimals=2)}  strouhal_number = {np.around(strouhal_number, decimals=2)}"
        )
        axes1[0].legend()
    ...
    if coeffs_figure:
        ...
        axes2[0].plot(
            Cp_validation_data["Lee(EXP)"][::2],
            Cp_validation_data["Lee(EXP)"][1::2],
            label="Lee(EXP)",
        )
        axes2[0].plot(
            Cp_validation_data["Bearman(EXP)"][::2],
            Cp_validation_data["Bearman(EXP)"][1::2],
            label="Bearman & Obasaju(EXP)",
        )
        ...
        f1 = interp1d(
            Cp_validation_data["Lee(EXP)"][::2], Cp_validation_data["Lee(EXP)"][1::2]
        )
        f2 = interp1d(
            Cp_validation_data["Bearman(EXP)"][::2],
            Cp_validation_data["Bearman(EXP)"][1::2],
        ...
        x_min = np.max(
            [
                np.min(Cp_validation_data["Lee(EXP)"][::2]),
                np.min(Cp_validation_data["Bearman(EXP)"][::2]),
            ]
        )
        x_max = np.min(
            [
                np.max(Cp_validation_data["Lee(EXP)"][::2]),
                np.max(Cp_validation_data["Bearman(EXP)"][::2]),
            ]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--path", default=os.getcwd(), type=str, help="Working directory"
    )
    parser.add_argument("-f", "--func1", type=str, help="choose utilities")
    shell_args = vars(parser.parse_args())

    const = {"D": 0.1, "U0": 1, "interval": [0.5, 1], "threshold": 0.25}
