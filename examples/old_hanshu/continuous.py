...
class Critic(nn.Module):
    ...
    def forward(
        ...
        logits, hidden,obs_all = self.preprocess(obs)    #处理的关键，初始定义的net网络， 与net中的forward函数的输出相同 网络层1 [(4，512),(512，128)]
        self.cishu+=1
        # print('2',self.cishu, obs.shape,logits.shape)    #sac learn()中运行四次  obs[256 4] -> [256,128]   4-> 128
        logits = self.last(logits)          #网络层2   [256, 1]   处理两次logits
        # print('3', self.cishu, logits.shape)  # sac learn()中运行四次  [256,128]  *[128 1] 变为 [256,1]  128-> 1
        return logits,obs_all
...
class ActorProb(nn.Module):
    ...
    def __init__(
        ...
        self.output_dim = int(np.prod(action_shape))
        input_dim = getattr(preprocess_net, "output_dim", preprocess_net_output_dim)
        # print(input_dim,self.output_dim)   # 128 1
        ...
        # print(type(self.mu),self.mu)    #[128, 1]
    ...
    def forward(
        ...
        logits, hidden,obs_all = self.preprocess(obs, state)   #网络层1 [(3，512),(512，128)]
