...
class OpenFoam_tianshou(gym.Env):
    ...
    def __init__(
        self, foam_root_path, foam_params, agent_params, state_params, server=True
    ):
        super().__init__()
        ...
        # Initialize displacement in the current time step
        ...
        # Initialize full cycle displacement
        self.history_displacement_df = pd.DataFrame()
    ...
    def step(self, actions: np.ndarray):
        ...
        self.displacement_df = utils.read_foam_file(
            self.foam_root_path + "/postProcessing/displacement",
            dimension=self.foam_params["num_dimension"],
        )
        ...
        if self.num_trajectory < 1.5:
            ...
            self.history_displacement_df = self.displacement_df
        else:
            ...
            self.history_disaplacement_df = pd.concat(
                [self.history_displacement_df, self.displacement_df]
            ).reset_index(drop=True)
    ...
    def reward_function(self):
        ...
        drug_coeffs_sliding_average = self.force_coeffs_sliding_average(
            vortex_shedding_period
        )[0]
        lift_coeffs_sliding_average = self.force_coeffs_sliding_average(
            vortex_shedding_period
        )[1]
        print(
            2 - drug_coeffs_sliding_average, 0.1 * np.abs(lift_coeffs_sliding_average)
        )
        return (
            2 - drug_coeffs_sliding_average - 0.1 * np.abs(lift_coeffs_sliding_average)
        )
    ...
    def reset(self):
        ...
        self.history_displacement_df.to_csv(
            self.foam_root_path + f"/record/displacement_{self.num_episode}.csv",
            index=False,
            header=False,
        )
        ...
        self.history_force_df = pd.DataFrame()
        self.history_force_Coeffs_df = pd.DataFrame()
        self.history_disaplacement_df = pd.DataFrame()
        self.info_list = []
    ...
    def force_coeffs_sliding_average(self, sliding_time_interval):
        ...
        if self.history_force_Coeffs_df.shape[0] <= sampling_num:
            sliding_average_cd = np.mean(
                signal.savgol_filter(self.history_force_Coeffs_df.iloc[:, 2], 11, 0)
            )
            sliding_average_cl = np.mean(
                signal.savgol_filter(self.history_force_Coeffs_df.iloc[:, 3], 11, 0)
            )
        else:
            sliding_average_cd = np.mean(
                signal.savgol_filter(
                    self.history_force_Coeffs_df.iloc[-sampling_num:, 2], 11, 0
                )
            )
            sliding_average_cl = np.mean(
                signal.savgol_filter(
                    self.history_force_Coeffs_df.iloc[-sampling_num:, 3], 11, 0
                )
            )
