...
def run(path, foam_params, writeInterval, deltaT, start_time, end_time):
    ...
    if foam_params['verbose']:
        ...
        mpi_process = subprocess.Popen(
            f'cd {path}' + ' && ' + foam_params['of_env_init'] + ' && ' + f'mpirun -np {foam_params["num_processor"]} {foam_params["solver"]} -parallel |tee log.pimpleFoam',
            shell=True, executable='/bin/bash'
        )
        ...
        subprocess.run(f'cd {path}' + ' && ' + foam_params['of_env_init'] + ' && ' + 'cat log.pimpleFoam | grep \'of mass\' | cut -d\' \' -f9 | tr -d \'(\'  > postProcessing/displacement',
            shell=True, check=True, executable='/bin/bash'
        )
    else:
        subprocess.run(
        ...
        mpi_process = subprocess.Popen(
            f'cd {path}' + ' && ' + foam_params['of_env_init'] + ' && ' + f'mpirun -np {foam_params["num_processor"]} {foam_params["solver"]} -parallel |tee log.pimpleFoam > /dev/null',
            shell=True, executable='/bin/bash'
        )
